...
> **Draft Note (1.1):** This is a **candidate** and carries no authority.
> **Frozen v1.0 remains the baseline** until the Owner approves and, separately,
> freezes this revision. `EDT F02` below is **allocated but not yet
> authoritative**, and no Story may cite it as Frozen until it is.
>
...
>
> **If the Owner intends the Platform registry regardless, this candidate is the
> cheapest possible place to say so**: it is a Draft, nothing cites `EDT F02`
> yet, and moving the allocation costs one edit rather than a superseding
> revision of two Frozen registries.
>
...
- **Status:** Draft
- **Version:** 1.1
- **Supersedes:** Nothing yet. **Frozen v1.0 remains the baseline** and would be
  preserved at `EDITORIAL_FEATURE_REGISTRY-v1.0-superseded.md` only if this
...
- `PLATFORM_FEATURE_REGISTRY.md` **Frozen v1.4** — the registry `F02` is
  deliberately **not** in. The Draft Note above gives the reason.
...
| 1.1     | 2026-09-08 | **Draft.** Allocates `F02` Editorial Review Authoring, the condition §2.1 set having been met by `PRD-0009` **Frozen v0.4** §13 and `PRD-0006` **Frozen v2.7** §22.2. Allocated here rather than in the Platform registry the Owner's instruction named, for the domain reason the Draft Note states in full. Every `PRD-0009` citation moves v0.3 → v0.4. Records the stale citation in `US-EDT-F01-001` at §5. |
...
**The recommendation is the second, and the Owner's decision.** It is recorded
