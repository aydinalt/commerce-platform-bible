...
> **`F11` and `F12` are now allocated.** `F12` Listing Reports is allocated with its behaviour owner still unwritten, and that is recorded rather than hidden: a Feature may be identified before its PRD section exists, and no Story may be written against it until one does.
...
> **It carries no Approval Note and no Freeze Note.** Frozen v1.0 remains
> authoritative until the Owner Freezes a successor, and **no Generated Story
> may consume `F11` or `F12` until then.**
>
> Two allocations, both for behaviour that is live and has no identity here:
>
> - **`F11` — Advertising Placement Settings.** `PRD-0006-platform.md` §20 was
>   added by v2.2 and approved on 2026-08-31 without a Feature ID, so the
>   platform's only settings capability for advertising has never been
>   identifiable. `PRD-0006-platform.md` v2.3 extends §20 with the
>   complementary-product region the platform serves itself.
> - **`F12` — Listing Reports.** The reader-report queue: an unverified claim by
>   anybody about one listing, and an Admin's decision to act on it or not.
>   Deliberately **not** part of `F02` General Moderation — a case is opened by
>   an Admin with authority and can change a target's state; a report is neither,
>   and folding them together would make the queue an Admin reviews
>   indistinguishable from a queue anybody can fill.
>
...
| F11        | Advertising Placement Settings       | Active       | Where advertising may appear and whether it appears at all.                          | `PRD-0006-platform.md` v2.3 §20                               | `UX-0006-admin-dashboard.md`; `UX-0003-offering-detail.md` §8.7          | No Capability Architecture required | Not required under ADR-0007          | The platform configures placement and serves the complementary region; it sells, prices, moderates and reports nothing.           |
| F12        | Listing Reports                      | Active       | A reader's report that something on one listing is wrong, and its review.            | **Pending** — no `PRD-0006-platform.md` section owns this yet | `UX-0006-admin-dashboard.md`; `UX-0003-offering-detail.md` §8.8          | No Capability Architecture required | Not required under ADR-0007          | Not a General Moderation case: opened by anybody, changes no target state, and produces an Admin judgement rather than an action. |
...
- **Behaviour owner reference:** `PRD-0006-platform.md` v2.3 §20
...
- **Behaviour owner reference:** **Pending.** No section of `PRD-0006-platform.md` owns this Feature. Until one exists, no Generated Story may be written against `F12`.
