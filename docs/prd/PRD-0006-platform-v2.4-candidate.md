...
> **Revision Note (2.4):** Superseding revision of Frozen v2.3, begun
> independently at Draft under a new version per `DOCUMENT_LIFECYCLE.md` §7.
> **It carries no Approval Note and no Freeze Note, because neither decision has
> been taken.** Frozen v2.3 remains the baseline and is preserved unchanged at
> `PRD-0006-platform.md` until this revision is reviewed, Approved and, if the
> Owner chooses, Frozen.
>
> This revision closes the governance debt the Owner listed on 2026-09-03:
> _"F12 için PRD bölümü ve saklama süresi"_ and the Feature Registry entries
> that depend on it. Three changes, and one of them is a defect in v2.3 itself.
>
> **(a) §12's list was wrong the day v2.3 was Frozen.** §12 enumerates where
> platform-owned operational rules may live and names five places. v2.2 added
> §20 Advertising Placement Settings — a sixth — and did not add it to the list,
> so the document simultaneously said "the platform's operational rules live in
> these five things" and "here is a sixth". The list is corrected. Nothing about
> §20 changes; what changes is that §12 now describes the document it is in.
>
> **(b) §20.4 says two things it always meant and never said.** Both were
> implemented on 2026-09-03 and neither was written down:
>
> - the **master switch suppresses the platform's own complementary region too**,
>   not only the external network's units. §20.6 already has a scenario for
>   "advertising is configured in all three regions"; the fourth region arrived
>   in v2.3 and the scenario was not extended with it, which left the one
>   control an operator reaches for in an incident with an undocumented gap;
> - a **Category exclusion is inherited downwards**, exactly as a Complementary
>   Placement is. An exclusion that stopped at the Category it names would leave
>   a sector marked ad-free advertising in every heading beneath it, which is the
>   opposite of what marking a sector means.
>
> **(c) §21 is new, and it is the behaviour owner `F12` has been waiting for.**
> `PLATFORM_FEATURE_REGISTRY.md` v1.1 allocated `F12` Listing Reports with its
> behaviour-owner reference recorded as **Pending**, and stated that no Story may
> be written against it until a PRD section exists. This is that section. It
> describes behaviour that has been live since 2026-09-03 and had no owner —
> which is the wrong order, and is recorded as such rather than tidied away.
>
> §21 also carries the **retention period** the Owner asked for. It is stated as
> a proposal in this Draft: a report holds a member of the public's own words,
> and how long the platform keeps them is the Owner's decision rather than an
> engineering default. §21.5 names the proposed period and the reasoning, and it
> is the one thing in this revision that needs an answer rather than a review.
>
> §21 is appended rather than inserted, for the same reason §20 was: renumbering
> twenty sections would break every reference in the repository to make room for
> one.
...
- **Freeze Date:** Not Frozen
- **Frozen By:** Not Frozen
- **Supersedes (proposed):** Frozen v2.3, which remains authoritative at `PRD-0006-platform.md` until this revision is Approved and Frozen
...
**§20 was missing from this list from 2026-08-31 until this revision.** v2.2
...
67. Platform shall delete a reviewed report, and the words it carries, at the end of the §21.5 retention period.
...
### 21.5 Retention — **proposed, and the Owner's decision**
...
- an **Open** report is kept until it is reviewed. A queue that deleted work
  nobody had done would lose the report and the fact that it was never answered;
...
indefinitely for a purpose nobody can name. It is a proposal, not a default: any
period the Owner names is implementable, and the number is recorded here so that
the decision is visible rather than buried in a sweep.

**Deletion is deletion.** The report is removed, not anonymised into a row that
survives forever without its content.
