...
> | `Hidden` | §7.2 makes it the outcome of an Admin's *Hide Offering* action, and FR-15 reserves the restore to an Admin. An intake producing Hidden would be an intake performing a moderation action, and a partner coming back would leave an Admin restoring four thousand listings by hand. |
...
> publicly ineligible *without changing its lifecycle state* — reversible,
...
- **Supersedes:** Frozen v4.0, preserved at `PRD-0001-offering-v4.0-superseded.md`
...
- it applies **only** to an Offering whose Source is Feed, and only through the
  intake that created that Offering. The boundary is §5.11.1's, restated here
  because this is the one place an intake touches something a person can see
  disappear;
...
Source is Feed AND the intake has not recorded the source as no longer offering it
→ Intake Availability Input = Eligible

Source is Feed AND the intake has recorded the source as no longer offering it
...
35a. An automated intake may set the Intake Availability Input only for an Offering whose Source is Feed and which that intake created.
...
Scenario: A vanished feed product stops being published without being retired
  Given a Published Offering whose Source is Feed
...
Scenario: An intake cannot reach an Offering it did not create
  Given a Published Offering whose Source is Business
  When any intake attempts to record it as no longer offered
