...
> **Freeze Note (3.1):** Explicitly Frozen by the Product Owner / Architecture
> Owner on 2026-09-26, **together with `US-DSC-F06-001` v1.2**, because §22.3
> here points at that Story for the depth bound and the Story's AC-13 would
> otherwise state a number no Frozen PRD sends it. Frozen v3.1 is the
> authoritative baseline; Frozen v3.0 is preserved at
> `PRD-0002-discovery-v3.0-superseded.md`. This exact version must not be edited
> in place; a further change requires a controlled revision under
> `DOCUMENT_LIFECYCLE.md` §7–§8.
>
> **Approval Note (3.1):** Explicitly approved by the Product Owner /
...
> taken as one decision on the same day. **The approval does not make the code
> conform** — §22.2's direct answer and §22.3's bound are both unbuilt, and the
> increment that builds them is separate from this one.
>
