...
| Current phase | M12 Increment I52 The Offering price — closed. Fifty-three increments, I0 through I52. **The first increment in this repository that began with a Frozen document rather than with code.** The Owner's decisions of 2026-08-30 — affiliate first, price belonging to the Offering, a product-matching key, an open Domain set — change what an Offering *is*, and an Offering is defined by PRD-0001, which was Frozen at v3.1. Writing the columns first would have made the schema the definition and the PRD its description, the exact inversion Documentation First Development exists to prevent; so the sequence was superseding revision → Owner approval → one code increment. **PRD-0001 is now Frozen v4.0.** The design turned on something the Owner supplied and the obvious model would have lost: *"ilan fiyatı olmayan ürünlerde olabilir örnek hizmet veriliyordur ama bir fiyatı yok — yönlendirme sonrası istenen hizmete göre belirlenen fiyat olabilir."* **There are two different absences of price** — a consultancy has no amount *by its nature*, and a feed that carried nothing has one the platform has not read — and a single nullable `price` makes them one row that every surface then has to guess about. Telling a person a service's price is unknown reports a failure where none occurred, which is the distinction PRD-0002 §14 already draws between zero results and results unavailable. So price is a **Kind first and an amount second**: `FIXED`, `ON_REQUEST`, `UNKNOWN`. Nine columns, **four CHECK constraints** and three indexes; `NUMERIC(12,2)` kept as text from column to JSON, because §5.10.5 makes the ordering of these amounts the product and an ordering computed from floats is occasionally and silently wrong. `Offering.source` records provenance so an intake can modify only what it created, and it is **absent from the write shape** so an owner cannot label their own Offering `FEED` and hand it to the intake. **The owner's edit form had to change in the same increment**: the write shape is a replacement, so an edit silent about price says the price is gone, and leaving the form alone would have made every routine save wipe one. **Two defects of mine, both caught by things the repository already had**: the raw price columns rode along on the read and `offeringContentSchema`'s `.strict()` turned every request into a `500` on the first try; and `$6` was deduced as the enum in one clause and `text` in another, a runtime failure no type-checker could name. **And one wasted hour worth recording** — I diagnosed that `500` as an SQL fault and proved the SQL fine, when the cause was a `packages/contracts/dist` from 24 August that the API was resolving; the chain's clean-tree rule exists for exactly that and I had skipped it while iterating. **The thirteenth wrong match, and the first that was mine rather than the repository's**: two substitutions in my own mutation harness matched nothing and reported the tests as *surviving*, which taken at face value would have led me to weaken two correct tests. 40 cases across two files, 13 mutations, every one killed. **The public surfaces still do not show price** — `ListingCard`, `SearchResult`, `OfferingPresentation` and `OfferingSearchProjection` are untouched, so nothing a visitor sees has moved, and that is the next increment. Also still absent: any feed infrastructure, so nothing can write `source = FEED`; a system Business, so Phase-1 Admin-authored Offerings have no owner; and Domains beyond the three seeded ones, which the PRD now permits. Increment I51 The page titles — closed. Fifty-two increments, I0 through I51. **Every browser tab said the wrong thing and no check had ever read one.** I27, I28 and I29 consolidated the application onto Turkish and each proved it with a detector that reads markup — text between tags, then, after five corrections, strings inside a JSX expression; `export const metadata` is neither. Measured across 22 routes: the site's own title was `Commerce Platform` while its header says **`İlanlar`** on every page, its description was `Decision-completion marketplace`, and two route titles were English. The clearest line of evidence is one file: `/account` renders `<h1>{TITLES.account}</h1>` — *Hesabınız* — **sixty lines below a title that said `Your account`**, with the Turkish string already sitting in the copy module since I27. The site names itself once now, from `BRAND.name`, so the tab cannot drift from the header. **And a second position no check reads was on screen**: `" (required)"` beside `Görünen ad` on Business Information since I28, which defeated `i27`'s expression scan twice over in one string — the pattern required a letter after the opening quote and this begins with a space, and parentheses were outside the permitted set. **The twelfth wrong match, and the sixth correction to that one detector**, and the hole was proven rather than asserted: reverting it with the English string back on screen leaves all fourteen of `i27`'s cases passing. The literal-title case asserts the **exact set** rather than testing for English, because `Karar` is Turkish in five ASCII letters and a heuristic would report the Decision flow's own title as a defect; the three remaining public literals are pinned rather than repaired, since extracting the public copy is the job I31 recorded and did not open. Increment I50 The Decision stages — closed. Fifty-one increments, I0 through I50. **The densest screen in the product was five files, 557 lines and not one `className`.** Its five sections now read as stages separated by a rule rather than by a card, because a card around a step makes the step look optional; the first is spared its rule, which would have been the second horizontal line in eighty pixels. **And colour was marking the container instead of the message**: `[role="alert"]` paints text critical and lands 56 times — 43 on a `<p>`, where that is right, and **four on a container**, including the Decision flow's "Devam edilemiyor" section whose heading, explanation and list of recovery links were *all* red. A container now takes a tinted surface and keeps critical ink for the refusal itself, and `--text` on `--critical-surface` was added to the measured contrast list rather than assumed. **The obvious repair was the overshoot** and a case guards it: narrowing the rule to `p[role="alert"]` would have broken forty-three things to mend four. **The eleventh wrong match, and the third of mine in three increments** — the case protecting the inline alerts was passed by the mutation it exists to catch, because `p[role="alert"] {` contains `[role="alert"] {`. The shape is now named: asking whether a substring is present in a large blob keeps failing where the blob holds near-identical strings, and asserting the exact set keeps working. Increment I49 The public surfaces — closed. Fifty increments, I0 through I49. **The public components were never the problem** — `.listing-card` carries thirteen rules and 29 of 31 classes were already referenced — but Home and Discovery had four sections, two navs and two lists with no structural treatment at all. Three patterns from existing tokens close it: an entrance at the prose measure, one navigation block for the three places that do the same act, and the results heading carrying the same rule `.workspace h1` carries, **asserted as an identical declaration** so the two halves of the product cannot drift apart. **A public block is deliberately not a workspace panel**: a results grid boxed in a card reads as a widget rather than as the page, so a case fails if either class grows a border or a radius. **I48's forcing function fired and was answered** — adding a class to Home failed the case asserting pages stay bare, and the argument was recorded rather than the list quietly widened. **Three versions of one case, and the ninth wrong match**: the guard on the public components was satisfied first by a plural covering for a singular — I28's collision, I29's answer — and then by a name appearing in five other selectors; it asserts the whole thirty-four-name vocabulary now. Increment I48 The management surfaces — closed. Forty-nine increments, I0 through I48. **Twenty of twenty-two `page.tsx` files carried no `className` at all.** I26 built the tokens and I33 turned the direction to dense listings, and both landed on the public surfaces; the seventeen an owner or an Admin works in were headings, paragraphs, forms and tables on a bare page. The site was never unstyled — it was typographically dressed and architecturally undressed, which is harder to notice than a plain page. **Nothing added is a new direction**: every rule is built from tokens that already existed, and `i26-design-foundation` reads the whole stylesheet, so each of the four mutations fails both suites rather than only the new one. **No page file was touched** — six one-element segment layouts carry the scope, and a case asserts the pages stay bare so a later class must be argued for. `.workspace ul:not([class])` keeps the row treatment away from `.listing-cards`, which a bare selector would have redrawn on the one screen where an owner compares their own listings with what a visitor sees. **Two of the new cases were wrong first**: one matched a `480px` breakpoint inside a comment explaining that it had been removed — the eighth time a check here has matched something other than what it meant — and the other used a pattern that matched the `2.75rem` it existed to permit. Increment I47 A refused connection — closed. Forty-eight increments, I0 through I47. **Every honest failure surface in the repository was unreachable in the plainest failure there is.** I23, I24, I45 and I46 are all reached by an `ApiRequestError`, and only a `5xx` and a timeout produced one — so an API that is simply not there threw a `TypeError` and took the page down. `/offerings/{slug}` answered `500` with a body whose **visible text was the page title alone**, and the crash screen was not in the document, because `error.tsx` is a client boundary and a server render that throws sends a shell. **The page was never wrong** — it has caught `isApiUnavailable` since I24, and one classification three files away made it unreachable, which is why the repair is in `fetchWithBudget` where all nineteen reads pass. A system call is the discriminator rather than a list of codes: a refused connection and a DNS failure carry `cause.syscall`, and the two ways this application can be wrong — unsupported scheme, malformed URL — do not. **Three smoke checks now drive a second web instance pointed at a closed port**, because `inject()` has no socket and `renderToStaticMarkup` has no status code; reverting the repair takes smoke to 15/17. **One mutation survived and the false claim was ours**: a case said the abort ordering was load-bearing, and an aborted `fetch` throws an `AbortError` rather than a `TypeError`, so the two paths are disjoint. Increment I46 The Decision reads — closed. Forty-seven increments, I0 through I46. **The last two modules that reported an outage as an answer**, and the two that lied hardest: `/decision` said *Bu karar akışının süresi doldu* and `/compare` said *Karşılaştırma oturumunuz sona erdi*, each following the false claim with an invitation to start again — so the cost was a Decision in progress and a Comparison Set built one Offering at a time, rather than a reload. The rule is now derived from the HTTP method rather than a list of names: a `GET` reports what is, everything else changes something, which is the same line I24 and I25 both drew and the reason these two modules were outside both rules at once. **The eight writes were measured and left alone** — their refusal copy says "nothing was initiated and no information was shared", true of an outage as of a refusal, and a case asserts it so a later increment does not "fix" correct copy. **The same socket probe read three ways, and only the third was true**: thirteen routes redirecting to sign-in was right for an anonymous visitor, the "you have nothing" copy was the cookie being absent, and only a request carrying the cookie reached the defect. I45 asserted the set of modules outside the vocabulary so that repairing them would fail there; both names are deleted and the set is empty. Increment I45 The identity reads — closed. Forty-six increments, I0 through I45. **An outage told a signed-in person they were signed out.** I24 taught thirteen routes UX-0006 §14's *distinguish zero from unavailable* and gave fourteen readers the vocabulary; `identity/api.ts` was not among them, so `readSession` turned a `503` into `null` and `/account` turned `null` into the sign-in screen — a form that calls the same API and would have failed too. `readOwnedBusinesses` was quieter and worse: it answered `{ businesses: [] }`, and because most people own none, **an outage wearing zero looked exactly like the truth**. Both reads were also outside I25's timeout budget, so a hung API held `/account` open with no ceiling; the eight writes sharing that function stay off it, which is I25's decision rather than an oversight. **The scope was corrected twice by measurement**: a socket probe made thirteen routes look wrong and could not tell, because it sent no cookie and a redirect to sign in is right for an anonymous visitor — twenty-one of twenty-two sites redirect only when there is no cookie at all. Counting readers then widened it: **fifteen Decision functions still collapse an outage into a confident nothing**, measured by driving them and asserted as an exact set. Increment I44 What the API answers — closed. Forty-five increments, I0 through I44. **The API is now driven against its own published description**, rather than the document being compared with the contracts a fourth time. It found two things neither file could show. `POST /decision/flows/{id}/direct-contact` answers `403 ORIGIN_MISSING` and declared `200 400 401 404 422` — the reveal carries a session, so ADR-0012 §2 applies, and it is the only Decision operation that carries one. And **`503` was declared on one operation out of eighty-seven**: `ErrorEnvelopeFilter` is an `APP_FILTER`, so answering `503 DEPENDENCY_UNAVAILABLE` belongs to every operation that reaches the database, and a generated client had no `503` branch anywhere but readiness. `GET /health/live` is the single exception and it is measured, not assumed — driven with nothing behind it, liveness answered `200` while readiness answered `503`. **73 response bodies were already correct.** The map from operation to contract is derived from the document itself, because all 379 declared responses point at a schema by `$ref` and none is inlined. Increment I43 The published types — closed. Forty-four increments, I0 through I43. **301 properties compared between the document and the contracts, all in agreement** — a lock-in rather than a repair. The first measurement said 62 differences and every one was the measurement's fault: three encodings of agreement read as disagreement, and trusting that number would have edited sixty-two correct declarations into wrong ones. Eight contracts carry a transform that JSON Schema cannot express; they are skipped and the set is asserted rather than a count, because a count is a budget somebody spends. Forty-three increments, I0 through I42. **The published contract had been hiding two fields since I30**: that increment gave Offerings visuals and updated the contracts, the database, the API and the web application, but not the five-thousand-line hand-written OpenAPI generator — so `SearchResult.primaryVisualUrl` and `EditableOfferingContent.visuals` were absent from the published description for eleven increments, and a generated client would have looked correct while dropping both. I41 could not see it because it compared method and path; CI's `git diff --exit-code` only ever proved the generator matches its own output. 81 of 92 schemas now pair with a Zod schema and are compared both ways. Forty-two increments, I0 through I41. **`generated/openapi.json` is 5073 hand-written lines and nothing compared it to the API**; CI's `git diff --exit-code` proves the generator matches the committed file and nothing else, and the only assertions named six operations out of eighty-seven. The document is in good order — 88 served, 87 documented, 0 invented, `/metrics` excluded on I19's decision — and that clean state is now held. **Mutation testing corrected a comment written in the same increment**: the route observer must precede `app.init()` rather than `NestFactory.create`. **A second smoke check passed for the wrong reason**: `/metrics` has never been a path, the real one is `/api/v1/metrics`. Forty-one increments, I0 through I40. **Nobody had ever pointed the platform at a brand-new database.** Walked against 31 migrations and nothing else: 39 tables, three Domains a migration seeds, Home saying "Şu anda açık bir kategori yok." rather than looking broken, and **I38's scheduled endpoint draining a real outbox for the first time**. The Admin panel answers 403 ADMIN_CONTEXT_REQUIRED to a granted account that has not entered the context, which is UX-0008 §5 working rather than a failure. The one thing missing was a way to get the confirmation link: the token is minted at delivery with only its digest written back, so no database access produces one, and `npm run first-run` is the worker run once with a dispatcher that prints instead of sends. Forty increments, I0 through I39. **A claim made earlier in this session was false**: the platform does throttle, and has since I13 — `auth_throttle` counts attempts per hashed subject across registration, recovery and both sign-in scopes, in one atomic statement shared by every instance. The survey that said otherwise searched for the names of libraries rather than for the behaviour. **What was broken is the key**: `request.ip` is only populated from `x-forwarded-for` when Fastify is told to, and it had not been told — so behind a proxy every caller shared one counter and the platform would lock itself out, while the obvious fix of trusting the whole chain would have counted a value the caller writes. `TRUSTED_PROXY_HOPS` states the hop count, and **too high is as bad as trust-all** because a chain shorter than the number declared resolves to the leftmost, forgeable entry. Thirty-nine increments, I0 through I38. **Until I38 no email was ever sent on the platform the Owner chose** — the worker is a `while (running)` loop and Vercel has nowhere to loop, so registration confirmations would have sat unread in the outbox, nobody could have completed a sign-up, and the deployment would have looked entirely healthy. The draining is now separated from what drives it: `main.ts` still loops, two scheduled endpoints call the same drain once, and it stops before a batch it could not finish because a function killed mid-batch is a delivery nobody recorded. **Vercel’s Hobby plan runs a cron once per day**, so the worker needs Pro or a process host — a spend decision rather than a code one. **I34’s environment detector was half-blind**: it matched `process.env.NAME` and not the bracket form, and a variable read only through a bracket would have been invisible to the check written to prevent exactly that boot failure. Thirty-eight increments, I0 through I37. **Vercel runs functions and until I37 the API had no way at all to run on the platform the Owner chose**: `main.ts` calls `listen` and never returns. `handler.ts` is a second entry rather than a second copy — `bootstrap.ts` had already separated building the application from listening on a port — and `main.ts` stays, because a staged decision is only reversible while both shapes exist. The application is built once per function instance and the promise is what is cached; building per request would open a new database pool on every call, which against Supabase’s connection limit is the whole project falling over. Two Vercel projects, because Vercel serves a root `api/` directory itself and a Next.js project already owns its routing. **The worker still has no home and it is now the largest gap**: until it runs no email is ever sent, so nobody can complete a sign-up, and a deployment made today would look healthy and be unusable. **No Vercel project exists and no Supabase instance has been created.** Thirty-seven increments, I0 through I36. **The Owner chose Vercel and Supabase on 2026-08-26, staged: ship on Vercel first and move the API to a process host if the measurements demand it.** A transaction pooler refuses the `options` startup parameter this platform's timeouts have travelled on since I18 — Supabase's port 5432 accepts it and port 6543 rejects it, and the code cannot tell them apart from the URL, so `DATABASE_CONNECTION_MODE` states which. On the pooled port the timeouts must come from the database role, and a deployment that forgets gets a working connection with **no statement timeout at all**; the API and worker now ask the server what the settings are and refuse to start when they are not the configured values. **The API still has no serverless entrypoint and the worker is still a 2-second poll loop** — neither runs on Vercel yet, and no Supabase instance has been touched. Thirty-six increments, I0 through I35. **Both services have now run as real processes and been driven over a socket, and the first run found a defect**: `/offerings/there-is-no-such-offering` answered 200 OK with "Bu sayfa bulunamadı" in the body, because a `loading.tsx` makes Next stream the segment and the status is committed at the first flush, before `page.tsx` calls `notFound()`. Twelve pages under `/admin`, `/businesses` and `/offerings/[slug]` were soft 404s and every test passed the whole time — `renderToStaticMarkup` has no status code and `app.inject()` does not stream. Three of I32's five `loading.tsx` files are removed; twelve pages lose a loading state because a correct status code outranks a skeleton. **All 942 tests before this one called `app.inject()` or `renderToStaticMarkup`** — no socket had ever been opened, no page had ever been served, and the web application had never called the API over a network. **Still nobody has looked at it**: `npm run smoke` drives HTTP and reads markup, with no browser, no client bundle and no viewport. **The repository can describe its own deployment**: `.env.example` documents all twenty-three variables the code reads instead of fourteen, `vercel.json` builds the web, and one Dockerfile carries the API and the worker — Vercel runs functions and both of those are processes. **Nothing has run**: no image built, no `vercel.json` read by Vercel, no migration applied to a hosted database, and the API's host is still unchosen. **Twenty-two routes existed and there was no site**: no header, no navigation, no footer, no brand mark — `layout.tsx` was `<html><body>{children}`. The Owner said "we still haven't moved to the frontend" twice and was answered twice with "the frontend already exists"; the Owner was right both times. **The Owner also replaced I26's approved calm direction with a dense listings one on 2026-08-24, and chose Vercel with managed Postgres as the hosting target** — no deploy artefact exists yet for any of the three services. **Eight Frozen sections name Loading Behaviour and there were zero `loading.tsx` files.** Five now cover the fourteen routes that wait; Home and Discovery are deliberately excluded, and Discovery's exclusion is an Owner question rather than a refactoring — UX-0002 §13 wants the criteria visible *and* the old actions inert, and neither option available today gives both. **Twenty-two routes had no error boundary and twenty-nine `notFound()` calls had no page**, so every uncaught error and every one of those calls produced Next.js's built-in English screen — including the thirteen `notFound()` answers I24 deliberately preserved. **Offerings carry visuals**: three Frozen acceptance criteria had been half-satisfied since the surfaces were built, because each says to present the supplied visual *and* not to invent one when absent, and only the second half was reachable. The Frozen documents call them *media* and *visuals*, which is why an earlier survey for the word *image* wrongly reported that the platform had none by decision. **I29's claim that all twenty-two routes speak Turkish was false** — sixteen English submit labels were on screen and all three consolidation increments passed over them. All twenty-two routes speak Turkish, corrected in I30; `<html lang="tr">` is true of the whole application and no route declares an exception. **Nothing can be deployed**: no `Dockerfile`, no deploy workflow, no named hosting target — see `docs/implementation/ROADMAP_2026-08-22.md`. **Nothing checks the OpenAPI document against the contracts** — `openapi.test.ts` asserts the health operations only, so the published description drifted from the schema silently. The first visual-design work in the repository: no Frozen UX document specifies visual design, so the Owner approved a foundation on 2026-08-21 and this implements it. The web application's calls to the API were the last untimed dependency edge in the repository, which made I23's and I24's surfaces unreachable in the failure that produces them most often. UX-0005 and UX-0006 join the public path; four of the eight Error Behaviour documents remain queued. R1.2, R1.4 and all of R2 of `docs/releases/V1_RELEASE_CRITERIA_CANDIDATE.md` need a hosting target and a monitoring system, neither of which exists |
...
I41 wrote down what it could not see — *"this compares method and path, not
shape"* — and **it did not match, and had not since I30.** That increment gave
...
migration seeds**; Home answers 200 with *"Şu anda açık bir kategori yok."*;
...
left the suite green, because the case asserted the source contained the *name*
...
contains `Herkese açık` — so an assertion that a withheld Offering is *not*
...
pressed *Giriş*, and arrived at **Sign in**. Findings are in
...
**The proposal's own contrast table was wrong.** It published *estimated* ratios
...
`if (!response.ok) return null;` appeared thirteen times, collapsing *this is not
here or not yours* and *the API did not answer* into one value; thirteen pages
