...
6. Write the two local schema checks that Known Boundaries claimed existed and did not: a relation-graph check over `schema.prisma`, and a foreign-key check asserting every key cascades on update except the one relation that overrides it. Neither is a substitute for the CI drift gate; both would catch the specific migration mistake — an inlined `REFERENCES ... ON DELETE` leaving `ON UPDATE` at `NO ACTION` — before a push rather than after one.
...
- A migration must match Prisma's generated names *and* spell both referential actions on every foreign key. An inlined `REFERENCES ... ON DELETE` leaves `ON UPDATE` at PostgreSQL's `NO ACTION` while the datamodel means `CASCADE`. This line previously claimed two local checks stood in for the gates that cannot run here — a relation-graph check over `schema.prisma` and a foreign-key check on update actions. **Neither exists:** no test and no script in this repository reads `schema.prisma`. The drift gate in target CI is the only thing that catches this.
