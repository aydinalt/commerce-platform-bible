...
| Current phase                     | **Five Frozen documents were reconciled and Home took Tailwind (I54).** The 2026-08-31 filter-as-you-type revision had left three other Frozen documents forbidding what it permitted, and a fourth contradicting itself — `US-DSC-F02-001` kept a BDD scenario asserting the behaviour its own AC-1 had just stopped requiring. That gap was mine: the round swept two owners of the rule and did not ask which others existed. All five are now Frozen together (PRD-0002 v2.3, UX-0001 v1.1, UX-0002 v1.1, US-DSC-F01-001 v1.1, US-DSC-F02-001 v1.2) with predecessors preserved. **Tailwind v4 is then installed in `apps/web` for the Home route only, deliberately without Preflight** — the usual single-line import would have reset all twenty-two routes invisibly on a commit scoped to one page. Tokens bridge through `@theme inline`, so no colour is written down twice. Home's behaviour is unchanged: UX-0001 §6 lists no Results and the prototype's `/` is a results screen, so only its visual language came. `.search-entry-row` is deleted; `.entry` and `.entry-nav` stay because Discovery still applies them. **Next: Discovery**, which retires both. Not proven locally: `next build` (SIGBUS in the sandbox), so CI is the first place the Next build sees Tailwind. |
