...
| Current phase                     | **Discovery is the second route on Tailwind and nine rule blocks left with it (I55).** Its page chrome — entrances, Category rows, results headings, Zero Results, the Compare-preparation notice — is written in utilities; `.entry`, `.entry-nav`, `.category-choices`, `.results-heading`, `.zero-results` and `.preparation-notice` are gone, with two `:has()` layout hooks. **One of those hooks was a trap**: it widened any `main` containing `.category-choices`, and deleting the class without noticing would have silently narrowed a Browse _branch_ page — the one case with Category choices and no Results. Found by reading the stylesheet, not by a test. Discovery's Listing Card was **deliberately not moved**, because Compare hand-writes the same markup and the card would have acquired two appearances. No behaviour changed: UX-0002 v1.1 permits filter-as-you-type and nothing implements it yet. I49's public-architecture cases were **re-pointed rather than deleted**, and a mutant caught the one claim I dropped while re-pointing them. **Next: Compare**, which retires the three `.listing-card` rules and lets Discovery's card move with it. Twenty routes still hold hand-written rules. |
